...
#ifdef ANDROID 
static const char TAGS[] = "wpa_supplicant";
static inline int wpa_lv2android(int level)
{
	switch (level) {
	case MSG_ERROR:
...
#endif /* CONFIG_DEBUG_FILE */
#ifdef ANDROID
        __android_log_vprint(wpa_lv2android(level), TAGS, fmt, ap);
#else
		vprintf(fmt, ap);
		printf("\n");
#endif /* ANDROID */
			 ...
			 size_t len, int show)
{
...
#endif /* CONFIG_DEBUG_FILE */
#ifdef ANDROID
	level = wpa_lv2android(level);
	__android_log_print(level, TAGS, "%s - hexdump(len=%lu): %s", title, (unsigned long)len, 
	...
	if (buf) {
		...
		while (i<len) {
			...
			__android_log_write(level, TAGS, line);
		}
		__android_log_print(level, TAGS, "--------------------------------------------------------------------------------");
	} 	
#else
