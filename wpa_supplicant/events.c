			      ...
			      struct wpa_ssid **selected_ssid)
{
	...
	u8 wapi_ie_len;
#endif
	wpa_printf(MSG_INFO, "Try to find WPA-enabled AP");
	for (i = 0; i < wpa_s->scan_res->num; i++) {
		...
		wpa_printf(MSG_INFO, "%d: " MACSTR " ssid='%s' "
		...
		wapi_ie = wpa_scan_get_ie(bss, WLAN_EID_WAPI); //WLAN_EID_WAPI
		wapi_ie_len = wapi_ie ? wapi_ie[1] : 0; 
		wpa_printf(MSG_INFO, "%d: " MACSTR " wapi_ie_len=%lu ",
                           (int) i, MAC2STR(bss->bssid),
		...
		if (e && e->count > 1) {
			wpa_printf(MSG_INFO, "   skip - blacklisted");
			continue;
		}
		...
		if (wpa_ie_len == 0 && rsn_ie_len == 0
			...
			wpa_printf(MSG_INFO, "   skip - no WPA/RSN IE");
			continue;
		}
		...
		for (ssid = group; ssid; ssid = ssid->pnext) {
			...
			if (ssid->disabled) {
				wpa_printf(MSG_INFO, "   skip - disabled");
				continue;
			}
			...
			wpa_printf(MSG_INFO, "   selected WPA AP "
				   MACSTR " ssid='%s'",
				   MAC2STR(bss->bssid),
				   wpa_ssid_txt(ssid_, ssid_len));
			*selected_ssid = ssid;
				  ...
				  struct wpa_ssid **selected_ssid)
{
	...
	wpa_printf(MSG_INFO, "Try to find non-WPA AP");
	for (i = 0; i < wpa_s->scan_res->num; i++) {
		const u8 *ssid_;
		u8 wpa_ie_len, rsn_ie_len, ssid_len;
		bss = wpa_s->scan_res->res[i];
		...
		for (ssid = group; ssid; ssid = ssid->pnext) {
			...
			wpa_printf(MSG_INFO, "   selected non-WPA AP "
				   MACSTR " ssid='%s'",
				   MAC2STR(bss->bssid),
				   wpa_ssid_txt(ssid_, ssid_len));
			*selected_ssid = ssid;
			  ...
			  struct wpa_ssid **selected_ssid)
{
	...
	wpa_printf(MSG_INFO, "Selecting BSS from priority group %d",
		   group->priority);
...
req_scan:
	if (wpa_s->scan_res_tried == 1 && wpa_s->conf->ap_scan == 1) {
		 ...
		 * to INACTIVE state.
		 */
		wpa_printf(MSG_INFO, "No network enabled Change state to WPA_INACTIVE");
