...
{
	if (eloop_is_timeout_registered(wpas_wps_timeout, wpa_s, NULL)) {
		wpa_msg(wpa_s, MSG_INFO, WPS_EVENT_TIMEOUT "Requested operation canceled");
		wpas_clear_wps(wpa_s);
		return 0;
	}
	return -1;
			      ...
			      struct wpa_ssid *ssid)
{
	...
	for (i = 0; i < wpa_s->scan_res->num; i++) {
		...
		if (sel_uuid == NULL || uuid == NULL ||
            ...
            /* EV# 63372 fix- stop wps on pbc overlap */
            wpa_msg(wpa_s, MSG_INFO, "STOPPING WPS DUE TO PBC SESSION OVERLAP.");
            wpas_clear_wps(wpa_s);
			ret = 1; /* PBC overlap */
			wpabuf_free(ie);
			break;
